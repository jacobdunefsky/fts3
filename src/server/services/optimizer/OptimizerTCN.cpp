...
float TCNOptimizer::explorationProbability()
{
    if (started) {
        float base_prob = exp(-(stepCount - lastActiveChange)/explorationDecaySteps) * maxExplorationProbability;
...
{
    cout << "conditions for pair: " << conditions[pair] << endl ;
    cout << "per connection pnalty: " << float(state.throughput >= conditions[pair]) * (conditions[pair] - state.throughput) << std::endl;
    return float(state.throughput >= conditions[pair]) * \
            (conditions[pair] - state.throughput);
}
...
//
TCNOptimizer::TCNOptimizer(float alpha,
